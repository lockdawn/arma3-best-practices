# Rifleman

## 📌 Descripción del Rol

El **Fusilero (Rifleman)** es el operador de infantería estándar dentro de la escuadra. Su función principal es proporcionar fuego directo, mantener la seguridad del equipo y apoyar las maniobras del elemento durante avances, defensas, repliegues o limpieza de zonas.

Este rol opera integrado con su escuadra, cubriendo sectores asignados, respondiendo a contactos enemigos y apoyando a roles especializados cuando sea necesario. Su configuración se mantiene ligera y funcional, utilizando únicamente uniforme y chaleco, sin mochila, lo que favorece la movilidad y reduce la fatiga durante la operación.

---

### 🧩 Mod Requerido
- **Arma 3 Workshop:** [ACE](https://steamcommunity.com/sharedfiles/filedetails/?id=463939057)

---

### 🩺 Rol ACE Requerido

- **ACE Rol:** N/A
- No necesita de roles ACE para desempeñar sus actividades.

<p align="center">
<img width="357" height="479" alt="image" src="https://github.com/user-attachments/assets/70e6f263-8263-43d2-8fc7-f506c35f7b8c" />
</p>

---

## 👕 Uniforme

### 📖 Descripción

El uniforme contiene el equipo básico de supervivencia y primeros auxilios. Estos elementos permiten al operador realizar intervenciones inmediatas en caso de emergencia, incluso antes de que el médico despliegue equipo especializado.

### ⚖️ Peso estimado

`~ 1.63 kg / 3.59 lb`

### 📦 Equipamiento

| Cantidad | Objeto |
|--------|--------|
| 2 | Earplugs |
| 1 | Fulton MX-991 |
| 2 | 16g IV |
| 10 | Bandage (Elastic) |
| 10 | Bandage (Packing) |
| 2 | Chest Seal |
| 1 | Combat Pill Pack |
| 2 | Eye Washer |
| 2 | Guedel Tube |
| 1 | Hand Warmer |
| 1 | Plasma IV (250ml) |
| 1 | Saline IV (250ml) |
| 2 | Splint |
| 4 | Tourniquet (CAT) |

---

## 🦺 Chaleco

### 📖 Descripción

El chaleco contiene el equipo táctico de combate inmediato, incluyendo munición, herramientas de control y elementos de señalización. Este equipamiento permite al operador defenderse y operar dentro del combate activo.

### ⚖️ Peso estimado

`~ 5.78 kg / 12.75 lb`

### 📦 Equipamiento

| Cantidad | Objeto |
|--------|--------|
| 6 | Rifle Ammunition (primaria) |
| 2 | Handgun Ammunition (secundaria) |
| 2 | Fragmentation Grenade |
| 2 | Smoke Grenade (white) |
| 1 | Smoke Grenade (green) |
| 1 | Smoke Grenade (purple) |
| 1 | Smoke Grenade (red) |
| 5 | Cable Tie (cinchos / esposas) |

---

## Nota Final

El Fusilero (Rifleman) es un rol esencial para mantener la cohesión, seguridad y capacidad de combate de la escuadra. Una configuración ligera y bien equilibrada permite que el operador se mantenga efectivo durante toda la misión sin comprometer movilidad ni respuesta táctica.

---

# Explosive Ordnance Disposal

## 📌 Descripción del Rol

El **Especialista en Explosivos (EOD – Explosive Ordnance Disposal)** es responsable de identificar, evaluar y neutralizar amenazas explosivas durante la operación. Su función principal es garantizar la seguridad del equipo mediante la localización y desactivación de artefactos explosivos improvisados (IED), minas, municiones sin detonar (UXO) y cualquier otro riesgo asociado a explosivos.

Este rol es crítico en misiones multijugador, ya que permite abrir rutas seguras, asegurar zonas de operación y prevenir bajas catastróficas que podrían comprometer toda la misión. La intervención oportuna del EOD reduce significativamente el riesgo operativo y mantiene la movilidad y continuidad de la unidad.

El especialista EOD debe estar equipado con herramientas especializadas para detección y desactivación, contar con entrenamiento técnico en procedimientos de manejo de explosivos y tener la capacidad de operar bajo presión en entornos hostiles, donde la precisión y la disciplina son fundamentales para evitar detonaciones accidentales.

---

### 🧩 Mod Requerido
- **Arma 3 Workshop:** [IEDD Notebook](https://steamcommunity.com/sharedfiles/filedetails/?id=3048818056)

---

### 🩺 Rol ACE Requerido

- **ACE Rol:** EOD
- Capacidad de utilizar equipo especializado IEDD.

<p align="center">
<img width="361" height="479" alt="image" src="https://github.com/user-attachments/assets/aeda8133-d5bf-4022-ada2-6bf4b399091e" />
</p>

---

## 👕 Uniforme

### 📖 Descripción

El uniforme contiene el equipo básico de supervivencia y primeros auxilios. Estos elementos permiten al operador realizar intervenciones inmediatas en caso de emergencia, incluso antes de que el médico despliegue equipo especializado.

### ⚖️ Peso estimado

`~ 1.63 kg / 3.59 lb`

### 📦 Equipamiento

| Cantidad | Objeto |
|--------|--------|
| 2 | Earplugs |
| 1 | Fulton MX-991 |
| 2 | 16g IV |
| 10 | Bandage (Elastic) |
| 10 | Bandage (Packing) |
| 2 | Chest Seal |
| 1 | Combat Pill Pack |
| 2 | Eye Washer |
| 2 | Guedel Tube |
| 1 | Hand Warmer |
| 1 | Plasma IV (250ml) |
| 1 | Saline IV (250ml) |
| 2 | Splint |
| 4 | Tourniquet (CAT) |

---

## 🦺 Chaleco

### 📖 Descripción

El chaleco contiene el equipo táctico de combate inmediato, incluyendo munición, herramientas de control y elementos de señalización. Este equipamiento permite al operador defenderse y operar dentro del combate activo.

### ⚖️ Peso estimado

`~ 5.24 kg / 11.55 lb`

### 📦 Equipamiento

| Cantidad | Objeto |
|--------|--------|
| 6 | Rifle Ammunition (primaria) |
| 2 | Handgun Ammunition (secundaria) |
| 2 | Fragmentation Grenade |
| 2 | Smoke Grenade (white) |
| 1 | Smoke Grenade (green) |
| 1 | Smoke Grenade (purple) |
| 1 | Smoke Grenade (red) |
| 5 | Cable Tie (cinchos / esposas) |

---

## 🎒 Mochila

### 📖 Descripción

La mochila está reservada para equipo especializado. En el caso del operador EOD, contiene las herramientas necesarias para la identificación, evaluación y neutralización de amenazas, así como equipo de apoyo para operar en entornos de alto riesgo.

Incluye dispositivos de detección, herramientas de desactivación, cargas de demolición controlada y equipo técnico que permite al especialista asegurar rutas, limpiar zonas de operación y reducir riesgos para el resto del equipo.

### ⚖️ Peso estimado

`~ 7.94 kg / 17.5 lb`

### 📦 Equipamiento

| Cantidad | Objeto |
|--------|--------|
| 2 | M112 Demolition Block |
| 1 | M183 Demolition Charge Assembly |
| 1 | Defusal Kit |
| 1 | Entrenching Tool |
| 1 | M57 Firing Device |
| 1 | Mine Detector |
| 2 | IEDD Notebook |
| 10 | Marker Flag (Red) |
| 1 | Spray Paint (Red) |

---

## Nota Final

El Especialista en Explosivos (EOD) es un rol crítico que requiere una configuración adecuada de equipo y herramientas especializadas para operar con seguridad y eficacia. Una mala preparación o equipamiento incompleto puede derivar en detonaciones accidentales, bajas masivas o la pérdida de control del área de operaciones, comprometiendo directamente toda la misión.

Este documento establece los lineamientos necesarios para garantizar que el operador EOD cuente con las capacidades técnicas y logísticas indispensables para la identificación, manejo y neutralización de amenazas explosivas en entornos operativos complejos.

---
